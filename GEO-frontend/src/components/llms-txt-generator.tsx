...
export function LlmsTxtGenerator() {
    ...
    // Clean up polling on unmount and fetch usage on mount
    useEffect(() => {
        fetchToolUsage();
        return () => {
    ...
    const isLimitReached = toolUsage?.llms_txt >= 2;
            ...
            pollingRef.current = setInterval(async () => {
                try {
                    ...
                    if (statusData.status === "completed") {
                        ...
                        fetchToolUsage(); // Update usage after success
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-1.5">
                            ...
                            <span className="text-xs text-teal-600 bg-teal-50 px-2 py-1 rounded-full font-medium border border-teal-100">
                                Limit: 2 uses max {toolUsage ? `(${toolUsage.llms_txt}/2 used)` : ''}
                            </span>
                        </div>
                        <div className="flex gap-3">
                            <div className="relative flex-1">
                                ...
                                <input
                                    ...
                                    onKeyDown={(e) => e.key === "Enter" && !isGenerating && !isLimitReached && handleGenerate()}
                                    placeholder="https://example.com"
                                    disabled={isGenerating || isLimitReached}
                                    className="w-full pl-11 pr-4 py-3 rounded-xl border border-gray-200 bg-white text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-500 transition-all disabled:opacity-50 disabled:bg-gray-50 disabled:cursor-not-allowed"
                                />
                            </div>
                            {result ? (
                                ...
                                <Button
                                    ...
                                    disabled={isGenerating || !url.trim() || isLimitReached}
